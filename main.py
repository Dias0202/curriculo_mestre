...
def bloco_experiencias(self, titulo: str, exps: list):
    ...
    for exp in exps:
        ...
        #  BUG #6 CORRIGIDO: fallback de data quando so existe inicio
        meta = ""
        if inicio and fim:
            meta = f"{inicio} - {fim}"
        elif inicio:
...
“””
Pos-processa o JSON retornado pelo LLM:
- Sobrescreve cabecalhos com valores hardcoded (nunca confia no LLM para isso)
- Trunca identificacao.titulo para maximo 6 palavras
- Remove lixo obvio de campos que nao devem ter conteudo longo
“””
if not isinstance(cv, dict):
return cv
...
# 2. Sanitizar titulo: maximo 6 palavras
...
# BUG #2 e #4 CORRIGIDOS: cabecalhos com valores literais fixos + exemplos negativos explicitos
