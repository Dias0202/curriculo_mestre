...
class CurriculoHarvard(FPDF):
    def __init__(self):
        super().__init__()
        # Margens otimizadas para ATS de pagina unica (15mm)
    ...
    def lista_duas_colunas(self, itens: list):
        self.set_font("helvetica", "", 10)
        # Calcula a largura de meia pagina respeitando as margens
        col_w = (self.w - self.l_margin - self.r_margin) / 2
        
        for i, item in enumerate(itens):
            item_str = safe_string(item)
            if not item_str: continue
            
            txt = sanitize(f"- {item_str}")
            if i % 2 == 0:
                # Se for o ultimo item da lista e estiver na coluna impar, forca quebra de linha
                if i == len(itens) - 1:
                    self.cell(col_w, 5, txt, new_x="LMARGIN", new_y="NEXT")
                else:
                    self.cell(col_w, 5, txt, new_x="RIGHT", new_y="TOP")
            else:
                self.cell(col_w, 5, txt, new_x="LMARGIN", new_y="NEXT")
        self.ln(2)

def gerar_pdf(dados: dict, idioma: str) -> io.BytesIO:
    if not isinstance(dados, dict): dados = {}
    pdf = CurriculoHarvard()
    pdf.cabecalho_candidato(dados.get("contato", {}))

    # Definicao dinamica de titulos com base no idioma retornado pela IA
    idioma_lower = idioma.lower() if idioma else ""
    if "ingl" in idioma_lower or "english" in idioma_lower or "en" == idioma_lower:
        h_res = "PROFESSIONAL SUMMARY"; h_exp = "EXPERIENCE"; h_edu = "EDUCATION"
        h_hab = "SKILLS"; h_cert = "CERTIFICATIONS"; h_idi = "LANGUAGES"
    elif "espan" in idioma_lower or "spanish" in idioma_lower or "es" == idioma_lower:
        h_res = "RESUMEN PROFESIONAL"; h_exp = "EXPERIENCIA"; h_edu = "EDUCACION"
        h_hab = "HABILIDADES"; h_cert = "CERTIFICACIONES"; h_idi = "IDIOMAS"
    else:
        h_res = "RESUMO PROFISSIONAL"; h_exp = "EXPERIENCIA"; h_edu = "FORMACAO"
        h_hab = "HABILIDADES"; h_cert = "CERTIFICACOES"; h_idi = "IDIOMAS"
    ...
    # Renderiza competencias e certificacoes em duas colunas compactas
    for tit, ch in [(h_hab, "competencias"), (h_cert, "certificacoes")]:
        lista = dados.get(ch, [])
        if isinstance(lista, str): lista = [lista]
        if lista and isinstance(lista, list):
            pdf.secao(tit)
            pdf.lista_duas_colunas(lista)

    idiomas = dados.get("idiomas", [])
    if isinstance(idiomas, str): idiomas = [idiomas]
...
def classificar_intencao_e_idioma_llm(texto) -> dict:
    p = (
        ...
        "2. 'idioma': Identifique o idioma em que o texto esta escrito (ex: 'Ingles', 'Portugues', 'Espanhol'). Se o usuario fez um pedido explicito de idioma, retorne o idioma solicitado.\n\n"
        f"TEXTO:\n{texto[:1500]}"
    )
    try:
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if u and u.get("email"):
        await update.message.reply_text("Bem-vindo de volta! Seu perfil esta configurado.\n\n"
                                        "Como usar:\n"
                                        "1. Envie ou cole seu Historico Profissional para atualizar sua base.\n"
                                        "2. Cole a Descricao da Vaga que voce deseja aplicar.\n\n"
                                        "O idioma e a formatacao ideal serao detectados automaticamente!")
        return ConversationHandler.END
    
    await update.message.reply_text("Ola! Eu sou seu Assistente de Curriculos ATS.\n\n"
                                    "Vou te ajudar a criar curriculos focados e otimizados para cada vaga de emprego, aumentando suas chances de contratacao.\n\n"
                                    "Para comecar, vamos configurar seus dados fixos.\n"
                                    "Qual o seu E-MAIL profissional?")
    return ASK_EMAIL

async def ask_email(u, c): 
    c.user_data['e'] = u.message.text
    await u.message.reply_text("Perfeito. Qual o seu numero de TELEFONE (com DDD)?")
    return ASK_PHONE

async def ask_phone(u, c): 
    c.user_data['t'] = u.message.text
    await u.message.reply_text("Otimo! Para finalizar, envie o link ou usuario do seu LINKEDIN.")
    return ASK_LINKEDIN
...
async def ask_linkedin(u, c):
    ...
    await u.message.reply_text("Perfil salvo com sucesso!\n\n"
                               "Agora a magica acontece. Siga os dois passos abaixo:\n\n"
                               "Passo 1: Me envie seu curriculo atual (pode ser arquivo PDF, TXT, ou colar o texto aqui) para eu criar sua base de dados.\n"
                               "Passo 2: Cole o texto da vaga de emprego que voce quer aplicar.\n\n"
                               "O idioma de saida sera detectado automaticamente baseado na vaga que voce enviar. Vamos la?")
    return ConversationHandler.END
...
async def handle_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if intencao == "HISTORICO":
            ...
            pdf = gerar_pdf(dados, idioma_detectado)
            nome = safe_string(dados.get("contato", {}).get("nome", "Candidato")).replace(" ", "_")
            await update.message.reply_document(document=pdf, filename=f"CV_{nome}_ATS.pdf", caption=f"Curriculo gerado em: {idioma_detectado}")
            await status.delete()

    except genai_errors.ClientError as e:
        if "429" in str(e) or "RESOURCE_EXHAUSTED" in str(e):
            logger.warning(f"Limite de cota da API atingido.")
            await status.edit_text("O servidor de Inteligencia Artificial esta sobrecarregado. Por favor, aguarde cerca de 1 minuto e envie sua mensagem novamente.")
        else:
        ...
        await status.edit_text("A Inteligencia Artificial falhou ao formatar o documento final. Por favor, tente enviar a vaga novamente.")
