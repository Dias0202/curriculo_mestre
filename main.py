...
4. CABECALHOS PADRONIZADOS (CRITICO): O objeto "cabecalhos" DEVE conter apenas titulos padroes de curriculo. NUNCA insira descricoes, resumos ou titulos de cargos neste objeto. Utilize termos simples traduzidos para o idioma alvo.
5. METODO STAR: Reescreva os bullet points de experiencias focando em impacto quantificavel.
6. ALAVANCAGEM DE BACKGROUND: Valorize Projetos Pessoais e trabalhos Freelance para compensar lacunas formais.
7. KEYWORDS OCULTAS: Liste termos exigidos pela vaga que o candidato NAO possui, para o stealth ATS.
8. FORMATO: Retorne JSON puro. Para listas de strings (como idiomas e certificacoes), retorne arrays com strings diretas (Ex: ["Ingles - Fluente"]), NUNCA insira objetos JSON dentro das listas simples.

SCHEMA OBRIGATORIO:
{
  "cabecalhos": {
    "resumo": "Texto exato: 'Resumo Profissional' (traduzido se necessario)",
    "competencias": "Texto exato: 'Competencias' (traduzido se necessario)",
    "experiencias": "Texto exato: 'Experiencia Profissional' (traduzido se necessario)",
    "educacao": "Texto exato: 'Formacao Academica' (traduzido se necessario)",
    "certificacoes": "Texto exato: 'Certificacoes' (traduzido se necessario)",
    "projetos": "Texto exato: 'Projetos' (traduzido se necessario)",
    "idiomas": "Texto exato: 'Idiomas' (traduzido se necessario)"
  },
  "identificacao": {
...
async def processar_e_enviar_vaga(
    ...
    if rel:
        ...
        if gaps:
            ...
            linhas.append("\nDICA DE GAPS: Se voce possui experiencia com alguma destas ferramentas (ou equivalentes), responda esta mensagem informando. O bot ira incorporar permanentemente ao seu banco de dados.")
